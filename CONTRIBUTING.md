...
| TYPE | 使うとき | 例 |
|------|---------|-----|
| `feat` | 新機能の追加 | `feat: 習慣チェック機能を追加` |
| `fix` | バグ修正 | `fix: ストリーク計算のズレを修正` |
| `style` | 見た目・デザインの変更 | `style: チェックアニメーションを調整` |
| `refactor` | 動作を変えないコード整理 | `refactor: useHabitsフックを整理` |
...
| 項目 | 形式 | 例 |
|------|------|----|
| ブランチ名 | `TYPE/english-brief-description` | `feat/habit-check-animation` |
| PRタイトル | ブランチ名と同じ | `feat/habit-check-animation` |
...
# 2. 作業ブランチを作成して移動する
# 例: git switch -c feat/add-habit-check
...
git add .
git commit -m "feat: 習慣チェック機能を追加"

# 5. リモートにプッシュする
# 例: git push origin feat/add-habit-check
...
# 9. 作業ブランチを削除する
# 例: git branch -d feat/add-habit-check
