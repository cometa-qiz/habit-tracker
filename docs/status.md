...
### 過去日の記録修正（TODO記入忘れ対応）
- [x] `src/utils/dateUtils.ts` に日付選択・曜日判定のユーティリティを追加する（今日以前のみ許可）
- [x] 過去日付を選べる日付ピッカーUIを実装する（未来日は選択不可）
- [x] 選んだ日付の「その曜日が対象の習慣」を一覧表示し、`HabitCard` で完了状態をトグルできるようにする
- [x] トグル時に `recordId = ${selectedDate}_${habitId}` の record を上書き更新する（新規作成しない）
- [x] 過去日を修正したあと、ストリーク・統計に正しく反映されることを確認する

### 統計カレンダーの月移動
- [x] `MonthlyCalendar.tsx` に「表示中の月」の state と前月／翌月ボタンを追加する
- [x] 表示中の月の records を取得して達成日を色付き表示する
- [x] 翌月ボタンが当月より先に進めないよう制限する

### データエクスポート（AIレビュー導線）
- [x] `users/{userId}` 配下の habits ＋ records をまとめて JSON でダウンロードする処理を実装する（クライアントサイド・Blob使用）
- [x] 同じデータを CSV 形式でもダウンロードできるようにする
- [x] エクスポートボタンを統計画面に配置する
- [x] 書き出したデータをAIに貼り付けてレビューしてもらうための手順表示（プロンプト例つき）を用意する

### ✅ 完了確認
- [x] `pnpm lint` がエラーなしで完了する
- [x] `pnpm build` が成功し `out/` が生成される
- [x] スマホ幅（375px）で週別グラフの日付が崩れない
- [x] 絵文字・カテゴリが候補選択／プルダウンで入力できる
- [x] 過去日のチェックを修正でき、ストリーク・統計に反映される
- [x] 統計カレンダーで前月・過去月を遡って表示できる
- [x] JSON / CSV でデータを書き出せる
- [x] `firebase deploy` でデプロイし公開URLで全機能を確認する
